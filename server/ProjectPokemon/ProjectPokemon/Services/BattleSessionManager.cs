...
{
    private readonly ConcurrentDictionary<Guid, BattleSession>
        _activeBattles = new();

    public BattleSession? GetBattle(Guid battleId)
    {
    ...
    public bool RemoveBattle(Guid battleId)
    {
    ...
    public BattleSession? GetBattleByUserId(int userId)
    {
        return _activeBattles.Values
            .FirstOrDefault(b => b.PlayerUserId == userId);
    }

    public IEnumerable<BattleSession> GetAllActiveBattles()
    {
        return _activeBattles.Values;
    }
}